#! /bin/bash
## -------------------- CHECK --------------------
command -v git &>/dev/null || ( (command -v dnf &>/dev/null && sudo dnf -y update && sudo dnf -y install git) || (command -v yum &>/dev/null && sudo yum -y update  && sudo yum -y install git) || (command -v apt-get &>/dev/null && sudo apt-get -y update && sudo apt-get install git) || (command -v brew &>/dev/null && brew update && brew install git) || (command "过程被中断,或者使用了不支持的包管理工具" && exit 1) )
...
( command -v yum &>/dev/null && (sudo yum -y install automake gcc gcc-c++ kernel-devel cmake python34-devel clang clang-devel) ) || ( command -v dnf &>/dev/null && (sudo dnf -y install automake gcc gcc-c++ kernel-devel cmake python34-devel clang clang-devel) ) || (command -v apt-get &>/dev/null && (sudo apt-get install build-essential cmake python-dev python3-dev clang clang-dev)) || (command -v brew &>/dev/null && (brew install cmake)) || (echo "过程被中断,或者使用了不支持的包管理工具" && exit 1)
## -------------------- PROCESS --------------------
